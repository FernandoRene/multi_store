...
class ProductosRepository {
  ...
  Future<bool> actualizar(int id, ProductosCompanion producto) async {
    return await (_database.update(_database.productos)
              ..where((p) => p.id.equals(id)))
            .write(producto) >
        0;
  }
  ...
  Future<bool> actualizarStock(int productoId, int nuevoStock) async {
    return await (_database.update(_database.productos)
            ...
            .write(ProductosCompanion(stockActual: Value(nuevoStock))) >
        0;
  }
  ...
  Future<List<String>> obtenerCategorias() async {
    ...
    return result
        .map((row) => row.read(_database.productos.categoria)!)
        .toList();
  }
  ...
  Future<EstadisticasGenerales> obtenerEstadisticas() async {
    try {
      ...
      final productosStockBajo = productosActivos
...
class VentasRepository {
  ...
  Future<int> insertarVentaCompleta(VentaCompleta ventaCompleta) async {
    return await _database.transaction(() async {
      // Insertar la venta
      final ventaId =
          await _database.into(_database.ventas).insert(VentasCompanion(
      ...
      for (final detalle in ventaCompleta.detalles) {
        await _database
            .into(_database.detalleVentas)
            .insert(DetalleVentasCompanion(
        ...
        await (_database.update(_database.productos)
              ..where((p) => p.id.equals(detalle.productoId)))
  ...
  Future<List<DetalleVentaCompleto>> obtenerDetallesVenta(int ventaId) async {
    try {
      final query = _database.select(_database.detalleVentas).join([
        leftOuterJoin(
            _database.productos,
            _database.productos.id
                .equalsExp(_database.detalleVentas.productoId))
      ])
  ...
  Future<bool> marcarComoSincronizada(int ventaId) async {
    return await (_database.update(_database.ventas)
...
final productosProvider =
    FutureProvider.family<List<Producto>, FiltroProductos?>(
        (ref, filtro) async {
...
final ventasProvider =
    FutureProvider.family<List<VentaCompleta>, FiltroVentas?>(
        (ref, filtro) async {
